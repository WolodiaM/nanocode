...
def call_api(model, messages, system_prompt, max_tokens=8192):
...
def main():
    ...
    while True:
        try:
            ...
            while True:
                response = call_api(model, messages, system_prompt, max_tokens)
                ...
                for block in content_blocks:
                    if block["type"] == "text":
                        log_event("assistant", text=block["text"])
                    ...
                    if block["type"] == "tool_use":
                        ...
                        log_event(
                            "tool", name=tool_name, arguments=tool_args, output=result
                        )
